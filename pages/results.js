    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {selectedScopes.map((scope) => (
          <div
            key={scope.branch}
            className="bg-gradient-to-br from-slate-50 to-indigo-50 rounded-xl border border-indigo-100 p-4"
          >
            <div className="flex items-center gap-2 mb-3">
              <span className="font-bold text-indigo-700 text-base">{scope.branch}</span>
              <span
                className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
                  scope.growth === "Very High"
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {scope.specializations.map((s) => (
                <span key={s} className="text-xs bg-white border border-indigo-200 text-indigo-700 px-2 py-0.5 rounded-full">
                  {s}
                </span>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {scope.companies.map((c) => (
                <span key={c} className="text-xs bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full font-medium">
                  {c}
                </span>
              ))}
            </div>
            ...
            <div className="text-xs text-slate-600 bg-white rounded-lg px-3 py-2 border border-slate-200">
              💰 Avg. Package:{" "}
              <span className="font-bold text-slate-800">{scope.avgPackage}</span>
            </div>
          </div>
        ))}
      </div>
...
function exportAsJSON(data, rank, category, branches) {
  const exportObj = {
    ...
    summary: {
      total: data.allSorted.length,
      dream: data.Dream.length,
    ...
    optionList: data.allSorted.map((d, i) => ({
      ...
      branch: d.branch,
      category: d.category,
      closingRank2025: d.closingRank,
      tier: d.tier,
      admissionProbability: `${d.probability}%`,
    })),
  };
  ...
  a.href = url;
  a.download = `KCET_Option_List_Rank${rank}.json`;
  a.click();
...
function exportAsText(data, rank, category, branches) {
  const lines = [
    ...
    "",
    "───────────────────────────────────────────────────────────",
    "  OPTION ENTRY LIST (in recommended preference order)",
    "───────────────────────────────────────────────────────────",
    "",
    ...data.allSorted.map((d, i) => {
      ...
      return (
        ...
        `       Branch: ${d.branch} | Category: ${d.category} | Cutoff: ${d.closingRank.toLocaleString()} | Probability: ${d.probability}%\n`
      );
    }),
    "═══════════════════════════════════════════════════════════",
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `KCET_Option_List_Rank${rank}.txt`;
  a.click();
  URL.revokeObjectURL(url);
}
...
export default function Results() {
  ...
  // router.query is empty on first SSR pass — wait for isReady
  ...
  useEffect(() => {
    ...
    const withRank = (arr) =>
      arr.map((d) => ({ ...d, rank: parseInt(rank, 10) }));
  ...
  }, [router.isReady, rank, category, branchesStr]);

  // ── Loading / no-query state ──────────────────────────────────────────────
  if (!results) {
    return (
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50">
        ...
        {/* ── Header ── */}
        <div className="hero-gradient text-white no-print">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div>
                ...
                <p className="text-indigo-200 text-sm mt-0.5">
                  Rank{" "}
                  <strong className="text-white">#{Number(rank).toLocaleString()}</strong>
                  {" · "}Category{" "}
                  <strong className="text-white">{category}</strong>
                  {" · "}Branches{" "}
                  <strong className="text-white">{branches.join(", ")}</strong>
                </p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => exportAsText(results, rank, category, branches)}
                  className="flex items-center gap-1.5 bg-white/20 hover:bg-white/30 text-white text-sm
                    font-semibold px-3 py-2 rounded-xl transition-all"
                >
                  📄 Export TXT
                </button>
                <button
                  onClick={() => exportAsJSON(results, rank, category, branches)}
                  className="flex items-center gap-1.5 bg-white text-indigo-700 text-sm
                    font-bold px-3 py-2 rounded-xl hover:bg-indigo-50 transition-all"
                >
                  📥 Export JSON
                </button>
              </div>
        ...
        <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1 no-print">
                    {tiers.map((tier) => {
                      ...
                      const inactive = {
                        Dream:  "bg-green-50 text-green-700 border border-green-300",
                        Target: "bg-blue-50 text-blue-700 border border-blue-300",
                        Safe:   "bg-orange-50 text-orange-700 border border-orange-300",
                        Backup: "bg-red-50 text-red-700 border border-red-300",
                      };
                      return (
                        <button
                          key={tier}
                          ...
                          className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold transition-all
                            ${activeTier === tier ? active[tier] : inactive[tier]}`}
                        >
                      ...
                      <div key={tier}>
                        ...
                        {results[tier].length === 0 ? (
                          <div className="text-center py-8 text-slate-400 bg-slate-50 rounded-xl">
                            ...
                            <p className="text-sm">No colleges in this tier for your rank and selection.</p>
                          </div>
                        ) : (
                          <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
                            {results[tier].map((college, i) => (
                              <CollegeCard
                                key={`${college.college}-${college.branch}`}
                                data={college}
                                index={i + 1}
                                showIndex={true}
                              />
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {results.allSorted.map((college, i) => (
                      <CollegeCard
                        key={`${college.college}-${college.branch}-${i}`}
                        data={college}
                        index={i + 1}
                        showIndex={true}
                      />
                  ...
                  {totalColleges === 0 && (
                    <div className="text-center py-12 text-slate-400">
                      ...
                      <p className="text-sm mt-1">
                        Try adjusting your rank, category, or branch selection.
                      </p>
                      <Link
                        href="/"
                        className="mt-4 inline-block text-indigo-600 font-semibold text-sm hover:underline"
                      >
              ...
              {activeTab === "compare" && (
                <div>
                  <h3 className="font-bold text-slate-800 text-base mb-1">
                    Cutoff Comparison (Top 5 Colleges)
                  </h3>
                  <p className="text-xs text-slate-500 mb-4">
                    Historical closing ranks of your top Dream and Target colleges vs your rank.
                  </p>
                  <ComparisonChart
                    colleges={[...results.Dream, ...results.Target].slice(0, 5)}
                    userRank={parseInt(rank, 10)}
                  />
                  {results.Dream.length + results.Target.length === 0 && (
                    <div className="text-center py-8 text-slate-400">
                      <p className="text-sm">
                        No Dream or Target colleges to compare. Select more branches or adjust your rank.
                      </p>
                    </div>
                  )}
                </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-4 text-xs text-slate-500">
            <strong className="text-slate-700">⚠️ Disclaimer:</strong> This tool uses simulated
            historical data for educational purposes only. Always verify with the official KEA
            website (kea.kar.nic.in) before submitting your option entry list.
          </div>
...
// Opt out of static generation — this page depends on client-side query params
